...
| 17 | "Encryption" silently no-ops when `ENCRYPTION_KEY` unset | **STILL OPEN** | `Encryptor.encrypt()` still returns plaintext unchanged when no Fernet key is configured (`if not self._fernet: return plaintext`), with only a warning logged. I haven't touched this one yet. |
