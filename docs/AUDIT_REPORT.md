...
**Method (re-audit):** Every status claim below was verified against the *current* code by direct read/grep — not inferred from memory of what was fixed. Where a finding is marked RESOLVED, PARTIALLY RESOLVED, or STILL OPEN, that reflects code actually re-read during this pass, not the remediation task list. Additionally, this pass included real execution — not just static review — that surfaced two previously-undetected fabrication/correctness bugs (see §0.3), fixed the same way everything below was: verify, fix, verify again with real output.
...
| 3 | GNN target leakage + fabricated molecular graphs | **PARTIALLY RESOLVED** | Target leakage is fixed: `_build_graphs()` now explicitly excludes the affinity column from input features, with a comment explaining the original bug. The graph fabrication itself (identical tiled node features, fully-connected fake topology, node count derived from MW not atom count) is **unchanged** — still not a real molecular graph. The module docstring now honestly discloses this limitation instead of staying silent about it. **Any GNN-derived accuracy claim is still not something this code can substantiate**, and the "19,520 Enamine compound" validation claim referenced in the original audit still has no corresponding dataset or run in this codebase. |
...
| 12 | Three mutually-inconsistent READMEs | **RESOLVED** | `README_RESEARCHER.md` and `README_DEVELOPER.md` are gone; only `README.md` remains, rewritten this pass with accurate current-state content (badges, features section, honest "why CEREBRO-X" framing that explicitly does not claim parity with commercial tools). |
...
| 15 | CORS wildcard + credentials | **PARTIALLY RESOLVED** | `CORS_ORIGINS` still defaults to `*` in `.env.example`/`docker-compose.prod.yml` (unchanged). But the specific *dangerous combination* the audit flagged (`allow_origins="*"` + `allow_credentials=True`) is now structurally prevented in code: `allow_credentials` is force-set to `False` whenever `CORS_ORIGINS` is unset/wildcard, with a warning log explaining why. |
...
| 17 | "Encryption" silently no-ops when `ENCRYPTION_KEY` unset | **STILL OPEN** | `Encryptor.encrypt()` still returns plaintext unchanged when no Fernet key is configured (`if not self._fernet: return plaintext`), with only a warning logged. Not touched this pass. |
...
| 21 | Zero test coverage of the scientific core / `cerebro_62_*` engines | **PARTIALLY RESOLVED** | Test count grew from 44 to 57 functions across 14 classes, adding real (non-tautological) coverage of `real_docking_engine.py` (LIE regression, pdb_id path-traversal), `pbbm_engine.py` predictors (pKa, permeability, pinned against a worked example), a full real end-to-end pipeline integration test, `cerebro_bbb_dnn.py`, `cerebro_dds_inverse_design.py`, the PDB resolver, rate limiting, and one `cerebro_value_resolver` category (`drug_smiles`, added this pass — see §0.3). **The `cerebro_62_*` engines themselves and the rest of `cerebro_value_resolver/` remain untested** — this specific gap from the original audit is not closed.

### 0.3 New findings from this pass — discovered via real execution, not static review

Static review can't catch everything; these two were found by actually running the full pipeline against real inputs (the same discipline this whole remediation pass was built around) and treating any anomaly in the output as worth chasing to its root cause rather than dismissing as noise.
...
The scientific-integrity gap the original audit called out as separate from the engineering bar is **narrower but not closed**: the GNN's fabricated graph structure is unchanged (target leakage specifically is fixed); the 62-criterion rubric's quantum-tunneling-adjacent entries (§4.5 below) were not revisited this pass; the deep-validation circularity is now honestly labeled with a separate `independent_pct` rather than eliminated. Two previously-undetected issues in the same spirit — a fabricated-statistics report panel and a drug-name-as-SMILES data-integrity bug — were found and fixed during this pass specifically *because* real execution was used to verify claims rather than trusting that "the code runs" meant "the code is correct." That discipline — run it for real, check the actual output, don't take a green build as proof of correctness — is the main methodological change this pass adds to the original audit's static-review approach, and it's worth continuing for whatever isn't covered yet (§0.2's remaining gaps, and anything not touched by this pass at all — §7 Performance, §10 Frontend/report-honesty, and the LOW-severity items in §13 were not re-verified here).

---

## Original Audit (2026-07-25, pre-remediation) — unmodified below
